...
- **Alpha propagation (FBIN)**: `draw()` carries a `_alpha` accumulator down the FBIN recursion tree. Each `is_mc=True` element's alpha multiplies into `_alpha` before the child `draw()` call; `_draw_image()` receives `_parent_alpha` and multiplies it with the leaf element's own alpha. This lets fade-in/fade-out effects authored on MovieClip instances (e.g. tree_105 `plantfood`/`plantfood2` body fade, `zombie_kungfu_hammer`) render correctly. RawBin alpha is not propagated this way — RawBin leaf elements already carry the final alpha directly.
